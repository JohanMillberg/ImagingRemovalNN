from forwardsolver_cpu import ForwardSolver
...
def generate_I_matrix(solver, image_index, N_x_im, N_y_im, N_t, N_s, plot, save):
	start = time.time()
	c = np.load(f"../images/labels/im{image_index}.npy")
	I = solver.calculate_intensity(c)
	
	if exists(f"./I_0_{N_t}_{N_s}.npy"):
		I_0 = np.load(f"./I_0_{N_t}_{N_s}.npy")
	else:
		R = np.load(f"./R_0_{N_t}_{N_s}.npy")	
	...
	if save:
		np.save(f"../images/data/im{image_index}.npy", I )
...
def main(argv):
	...
	n_fractures = 6
	fracture_width = 4
	buffer_size = 0 # space between fractures
	mean_noise = 1.0
	std_dev_noise = 0.2
	max_length = 50
	min_length = 20
